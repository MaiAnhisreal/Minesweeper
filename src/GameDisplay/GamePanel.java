...
public class GamePanel extends JPanel implements MouseListener{
	...
	public void mousePressed(MouseEvent e) {
		getP1().getBtButtonSmile().setStage(ButtonSmile.wow);
		getP1().getBtButtonSmile().repaint();
				...
				if(e.getButton() == 1 && e.getSource() == arrayButton[i][j]&&!world.getArrayCamco()[i][j]) {
					...
					if(!world.open(i,j)) {
						if(world.isComplete()) {
							...
							getP1().getTime().stop();
							getP1().getBtButtonSmile().setStage(ButtonSmile.lost);
							getP1().getBtButtonSmile().repaint();
							
							
				     int option = JOptionPane.showConfirmDialog(this, " You are lost, play again?"," ", JOptionPane.YES_NO_OPTION);
						if (option == JOptionPane.YES_OPTION) {
							...
							getP1().getTime().stop();
							getP1().getBtButtonSmile().setStage(ButtonSmile.win);
							getP1().getBtButtonSmile().repaint();
							
				     		  int option = JOptionPane.showConfirmDialog(this, " Congratulation!!!You are win, play again?"," ", JOptionPane.YES_NO_OPTION);
								if (option == JOptionPane.YES_OPTION) {
									gameFrame.setVisible(false);
									new GameFrame(w, h, boom);
								}
	...
	public void mouseReleased(MouseEvent e) {
		getP1().getBtButtonSmile().setStage(ButtonSmile.now);
		getP1().getBtButtonSmile().repaint();
	}
